...
RUN echo "Z"`,
	...
	for i := 0; i < 26; i++ {
		...
		c.Assert(actualValue, checker.Contains, echoValue, check.Commentf("Expected layer \"%s\", but was: %s", expectedValues[i], actualValue))
	}
...
func (s *DockerSuite) TestHistoryImageWithComment(c *check.C) {
	...
	c.Assert(actualValue, checker.Contains, comment, check.Commentf("Expected comments %q, but found %q", comment, actualValue))
}
